...
SELECT * from Authorship LIMIT 10
```
...
CREATE TABLE IF NOT EXISTS dimHistory(
  historyRef INTEGER AUTO_INCREMENT,
  datePublished TEXT,
...
SELECT History.pubStatus AS pubStatus,
  History.PMID AS PMID,
 History.day || "-" || History.month || "-" || History.year AS datePublished,
 History.month as quarter,
  CASE
...
#date1 <- parse_date(date1, format = "%d-/%m-/%Y")
PMID <- df_dimHistory$PMID
quarter <- df_dimHistory$quarter

df_pubDateStar <- data.frame(cbind(date1,quarter,pubstatus,PMID))
df_pubDateStar

#convert date columsn into one to date format for quarter
names(df_pubDateStar)

#df_pubDateStar <- df_pubDateStar %>% rename(pubstatus = pubStatus)
df_pubDateStar <- df_pubDateStar %>% rename(datePublished = dfTest.date)

head(df_pubDateStar)

dbWriteTable( star.dbs, "dimHistory", df_pubDateStar, field.types = NULL, row.names = FALSE, 
               overwrite = FALSE, append = TRUE, allow.keywords = FALSE )
# 
...
Create query that sorts by quarter - get from star schema
```{r}
