...
--SELECT * FROM dimPubDate

SELECT COUNT(factArticle.PMID) as numOfArticles, dimPubDate.quarter, year FROM factArticle 
JOIN dimPubDate ON dimPubDate.dateRef = factArticle.dateRef
GROUP BY dimPubDate.quarter, dimPubDate.year;
--JOIN dimAuthor ON factArticle.authorRef = dimAuthor.authorRef
--JOIN dimJournal ON factArticle.journalRef = dimJournal.journalRef
--JOIN dimHistory ON factArticle.historyRef = dimHistory.historyRef
...
viz <- ggplot(data = articlesBySeason, aes(x = quarter, y = numberOfArticles, group = 1 )) + geom_line( color = 'blue') + geom_point() + labs(title="Publications by Season", x="Season", y="Quantity of Articles")

viz

# viz <- ggplot(data = newData, aes(x = strikeYear, y = numOfStrikes, group = 1)) +
#   geom_point(aes(color=numOfStrikes), alpha=0.9) + geom_line() + 
#   labs(title= "Birds Strikes Incidents by Year ", subtitle="Years 2000 - 2011", x= "Year", y="Number of Bird Strikes")
...
Publication status (or activity) by season


```{r}
dbGetQuery(star.dbs, "SELECT * FROM factArticle 
           JOIN GROUP BY pubStatus")
```
...
sqlCmd = " SELECT quarter, COUNT(*) AS 'numOfarticles' FROM (SELECT pmid, quarter FROM dimPubDate
GROUP BY PMID) GROUP BY quarter"
#get mpid and quarter of each unique article


byQuarter <- dbGetQuery(star.dbs, sqlCmd)
byQuarter

dbGetQuery(star.dbs, "SELECT * FROM dimPubDate")

dbGetQuery(star.dbs, "SELECT * FROM factArticle JOIN dimPubDate ON dimPubDate.")
